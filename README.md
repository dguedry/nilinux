...
| Native Access quits within a second of starting ("GPU process launch failed: error_code=39", "Network service crashed", "GPU process isn't usable. Goodbye."), or its renderer dies at once and the wineserver then spins at 100% CPU | Chromium's Windows sandbox broker could not spawn child processes; seen once on a prefix in a bad state (every version affected, including one that had just worked), gone after *Re-run setup / repair* and a fresh prefix boot. Root cause not pinned down | Launches with `--no-sandbox` next to `--disable-gpu` so the broker is never needed; a version that is known not to work can be listed in `KNOWN_BAD` and Health, `status` and the Install tab then say so |
...
- Native Access 3.23.0, 3.25.2 and 3.26.0 (`KNOWN_GOOD` in `native_access.py`; 3.26.0 = Electron 43, daemon 1.32.0, its own dependency fix);
