...
| `nilinux sync` (Flatpak) fails with `OSError: [Errno 16] Device or resource busy: '~/.local/share/yabridge' -> '...yabridge.bak'` | The yabridge directory is a `--filesystem=` grant, so inside the sandbox it is a bind mount, and a mount point cannot be renamed | Installs by replacing the files inside the directory (each through a temporary name in the same directory, which also leaves a running DAW's mapped libraries alone); the replaced files are kept in `yabridge/previous` |
...
nilinux/
  ...
  urlhandler.py      native-access:// sign-in callbacks: a host .desktop handler
                     so a browser login returns to Native Access
