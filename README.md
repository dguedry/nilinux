...
| Mouse clicks land in the wrong place in *some* plugin GUIs even with the master build (IK Multimedia Hammond B-3X), shifted by the window's position on screen; moving the window fixes it until the next open | yabridge only tells Wine where the embedded window is when the host moves it; a plugin whose editor has its final size at once never triggers that, so Wine keeps the window at (0,0) | `patches/yabridge-*.patch` is applied on top of master by `scripts/build-yabridge.sh`: the position is pushed to Wine as soon as the window is mapped; worth sending upstream |
