...
class PluginWineTest(unittest.TestCase):
    def setUp(self):
        ...
        self.patches = [mock.patch.object(yabridge, "YAB_DIR", self.yab),
                        ...
                        mock.patch.object(yabridge, "LEGACY_SHIM", self.home / ".local/bin/wine"),
    ...
    def test_our_old_shim_removed_once_the_record_is_in_force(self):
        shim = self.home / ".local/bin/wine"
        _exe(shim, f"#!/bin/sh\n{yabridge.LEGACY_SHIM_MARK}\nexec wine\n")
        self.assertFalse(yabridge.remove_legacy_routing(self.prefix), "not before the record works")
        self.assertTrue(shim.exists())
        self.activate()
        self.assertTrue(yabridge.remove_legacy_routing(self.prefix))
        self.assertFalse(shim.exists())

    def test_a_wine_that_is_not_ours_is_never_touched(self):
        self.activate()
        shim = self.home / ".local/bin/wine"
        _exe(shim, "#!/bin/sh\necho someone else's\n")
        self.assertFalse(yabridge.remove_legacy_routing(self.prefix)); self.assertIn("someone else", shim.read_text())
        shim.write_bytes(b"ELF\xff\xfe\x00 a real wine, e.g. Proton on Bazzite")     # not UTF-8: must not raise either
        self.assertFalse(yabridge.remove_legacy_routing(self.prefix)); self.assertTrue(shim.read_bytes().startswith(b"ELF\xff"))
        shim.unlink(); shim.symlink_to(self.hostbin / "wine")                        # the user's own symlink
        self.assertFalse(yabridge.remove_legacy_routing(self.prefix)); self.assertTrue(shim.is_symlink())

    def test_our_old_symlink_and_environment_d_removed(self):
        shim = self.home / ".local/bin/wine"; shim.parent.mkdir(parents=True)
        shim.symlink_to(self.prefix.build.wine)                                       # the very first releases
        legacy = self.home / ".config/environment.d/50-nilinux.conf"; legacy.parent.mkdir(parents=True)
        legacy.write_text(yabridge.legacy_daw_environment_content(self.prefix))
        self.activate()
        self.assertTrue(yabridge.remove_legacy_routing(self.prefix))
        self.assertFalse(shim.exists() or shim.is_symlink()); self.assertFalse(legacy.exists())
        legacy.write_text("WINELOADER=/somewhere/else/wine\n")                        # not ours: kept
        self.assertFalse(yabridge.remove_legacy_routing(self.prefix)); self.assertTrue(legacy.exists())
