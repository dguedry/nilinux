...
NI_KEY = "Software\\Native Instruments\\"
LIMITS = ("Programs that need .NET, an embedded browser (Gecko) or Direct3D will not run here: "
          "this prefix is tuned for audio software and adding those layers would put the NI stack at risk.")
...
class Program:
    ...
    sources: list = field(default_factory=list)   # "registry", "shortcut"
    @property
    def runnable(self) -> bool: return bool(self.exe)
    @property
    def ni(self) -> bool: return "native instruments" in self.publisher.lower() or self.name.lower().startswith("native instruments")
...
def ni_programs(p: Prefix) -> list[Program]:
    ...
    for key, vals in sections.items():
        if not key.startswith(NI_KEY) or "\\" in key[len(NI_KEY):]: continue
        name = key[len(NI_KEY):]; d = vals.get("InstallDir", "")
...
def _find_exe(p: Prefix, install_dir: str) -> str:
...
def installed(p: Prefix) -> list[Program]:
    ...
    regs = registry_programs(p) + ni_programs(p); links = shortcut_programs(p)
    ...
    for l in links:
        ...
        for r in by_name.values():
            if _norm(l.name) == _norm(r.name) or _norm(l.name) == _norm("Native Instruments " + r.name) or _norm("Native Instruments " + l.name) == _norm(r.name) \
...
def find(p: Prefix, name: str) -> Program:
    ...
    if not part: raise LookupError(f"no installed program matches '{name}' (nilinux programs lists them)")
