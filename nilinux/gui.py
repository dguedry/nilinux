            ...
            def show():
                if v["installed_known_bad"]:
                    ...
                    self.na_update_row.set_subtitle(f"Installed {v['installed']} · {v['latest']} available "
                        + ("(validated on this stack). Download it from NI, then pick the file here." if v["latest_known_good"]
                           else (f"(does not work on this stack: {v['latest_known_bad']}). Stay on {v['installed']}." if v["latest_known_bad"]
                                 else "(not yet validated on this stack). Download it from NI, then pick the file here.")))
